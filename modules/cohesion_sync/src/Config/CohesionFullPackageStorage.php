...
class CohesionFullPackageStorage implements StorageInterface {
   ...
   *   The configuration manager.
   */
  public function __construct(StorageInterface $storage, ConfigManagerInterface $config_manager) {
    $this->storage = $storage;
    $this->configManager = $config_manager;
    $this->configSyncSettings = $this->storage->read('cohesion.sync.settings');
  }
  ...
  public function getIncludedEntityTypes(): array {
    if (!isset($this->includedEntityTypes)) {
      $enabled_entity_types = $this->configSyncSettings['enabled_entity_types'];
  ...
  public function listAll($prefix = '') {
    ...
    $site_studio_config = array_filter($site_studio_config, function ($name) {
    ...
    $data = array_unique(array_merge($site_studio_config, $required_drupal_config));
    sort($data);
